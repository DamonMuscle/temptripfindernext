(function()
{
	...
	DetailViewViewModel.prototype.getGridDefinitionByType = function(type)
	{
		...
		{
			case "aide":
				return tf.staffGridDefinition.gridDefinition().Columns;
			case "altsite":
				return tf.altsiteGridDefinition.gridDefinition().Columns;
			case "contractor":
				return tf.contractorGridDefinition.gridDefinition().Columns;
			case "district":
				return tf.districtGridDefinition.gridDefinition().Columns;
			case "driver":
				return tf.staffGridDefinition.gridDefinition().Columns;
			case "fieldtrip":
				return tf.fieldTripGridDefinition.gridDefinition().Columns;
			case "georegion":
				return tf.georegionGridDefinition.gridDefinition().Columns;
			case "school":
				return tf.schoolGridDefinition.gridDefinition().Columns;
			case "tripstop":
				return tf.tripStopGridDefinition.gridDefinition().Columns;
			case "staff":
				return tf.staffGridDefinition.gridDefinition().Columns;
			case "student":
				return tf.studentGridDefinition.gridDefinition().Columns;
			case "trip":
				return tf.tripGridDefinition.gridDefinition().Columns;
			case "vehicle":
				return tf.vehicleGridDefinition.gridDefinition().Columns;
			default:
				return;
		}
	};
 ...
 */
	...
	* @param {PrintDocument} printDoc
	 * @return {void}
	 */
	DetailViewViewModel.prototype.copyGSStyleToPrintDoc = function(printDoc)
	{
		var doc = printDoc._getDocument(),
			sheets = document.styleSheets, sheetsLength = sheets.length, gsSheet;

		for (var i = 0; i < sheetsLength; i++)
		{
			var sheet = sheets[i];
			if (sheet.cssRules.length > 0 && sheet.cssRules[0].cssText.indexOf('grid-stack-instance') >= 0)
			{
				gsSheet = sheet;
				break;
			}
		...
		var rulesLength = gsSheet.cssRules.length;
		for (var i = 0; i < rulesLength; i++)
		{
			if (typeof style.sheet.insertRule === 'function')
			{
				style.sheet.insertRule(gsSheet.cssRules[i].cssText, i);
			} else if (typeof style.sheet.addRule === 'function')
			{
				style.sheet.addRule(gsSheet.cssRules[i].selectorText, gsSheet.cssRules[i].style.cssText, i);
			}
		}
	};

	/**
	 * Add body styles to print document.
	 * @param {PrintDocument} printDoc
	 * @return {void}
	 */
	DetailViewViewModel.prototype.addStyleToPrintDoc = function(printDoc)
	{
		var addStyle = function(style)
		{
			var doc = printDoc._getDocument(), es = doc.createElement('style');
		...
		for (var i = 0; i < links.length; i++)
		{
			...
			if (link.href.match(/\.css$/i) && link.rel.match(/stylesheet/i))
			{
				...
				var linkRoot = link.href.replace(/\/[a-zA-Z\-\_\.\&]+\.css$/gi, '');
				cssText = cssText.replace(/url\(\"(\.\.\/[a-zA-Z0-9\-\_\/\.\&]+)\"\)/gi, 'url("' + linkRoot + '/$1")');
	 ...
	 * @return {void}
	 */
	DetailViewViewModel.prototype.clearPrintStyle = function(printDoc)
	{
		var doc = printDoc._getDocument(),
			sheetsLength = doc.styleSheets.length;

		// remove wrong font link, will add css at @media print.
		for (var i = 0; i < sheetsLength; i++)
		{
			...
			for (var j = 0; j < sheet.cssRules.length; j++)
			{
				...
				if (rule.cssText.indexOf('@font-face') >= 0)
				{
					...
					switch (rule.style.fontFamily)
					{
						case 'SourceSansPro-Regular':
						case 'SourceSansPro-SemiBold':
						case 'SourceSansPro-Italic':
						case 'SourceSansPro-Bold':
							sheet.deleteRule(j--);
							continue;
					}
				}
	...
	{
		var self = this,
			printDoc = new TF.PrintDocument(),
			DetailViewViewModel = $(e.target).closest('.detail-view-panel');

		printDoc.append(DetailViewViewModel[0]);
		...
		// PrintDocument copies style from head automatically.
		// But there's a bug that styles will be copied to body when running.
		// So style should be copied from body here.  Once this bug fixed,
		// please remove this method.
		self.addStyleToPrintDoc(printDoc);

		self.copyGSStyleToPrintDoc(printDoc);

		addLogo();

		// Default Print style is a bug that set a wrong page size for print document.
		// Once this style is removed, should also remove following code.
		var defaultPrintStyle = $(document).find('style[id="default-print"]');
		if (defaultPrintStyle)
		{
			var defaultPrintStyleInner = defaultPrintStyle.html();
			defaultPrintStyle.html('');
		}
		self.clearPrintStyle(printDoc);
		printDoc.print();

		if (defaultPrintStyle)
		{
			defaultPrintStyle.html(defaultPrintStyleInner);
		}
	};
