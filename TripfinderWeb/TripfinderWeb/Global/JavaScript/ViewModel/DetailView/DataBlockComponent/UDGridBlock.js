(function () {
	...
	UDGridBlock.prototype.getGridColumnsByType = function () {
		...
		for (var key in fieldNameAndGUID) {
			let isMapUDGridField = self.options.UDGridFields.findIndex((f) => { return f.Guid === key && f.type === "Map"; }) >= 0;
			if (isMapUDGridField) {
				originColumns.push({
		...
		let isEditableBasedOnSignedPolicy = tf.udgHelper.getEditableBasedOnSignedPolicy(self.options);
	...
	UDGridBlock.prototype.editClickEvent = function (e) {
		...
		let udfFields = kendoGrid.options.columns.filter(c => c.udfType == "SignatureBlock").map(d => { return { Type: 15, Guid: d.FieldName } });
		let isSigned = tf.udgHelper.getIsReadOnlyBasedOnSignedPolicy(UDGRecord, udfFields);
		let isReadOnly = this.isReadOnly() || isSigned;
		...
		if ($rows && $rows.length) {
			dataItems.forEach(function (dataItem, idx) {
				if ($rows.length >= idx + 1) {
					$editBtn = $($rows[idx + 1]).find('.k-button.k-button-icontext.k-grid-edit');
					$viewBtn = $($rows[idx + 1]).find('.k-button.k-button-icontext.k-grid-view');

					let udfFields = kendoGrid.options.columns.filter(c => c.udfType == "SignatureBlock").map(d => { return { Type: 15, Guid: d.FieldName } });
					let isReadOnly = tf.udgHelper.getIsReadOnlyBasedOnSignedPolicy(dataItem, udfFields);
					if (isReadOnly) {
