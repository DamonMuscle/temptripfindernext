(function()
{
	...
	function RoutingMapSearch(map, options)
	{
		...
		self.allTypes = [
			...
			{ text: "Alternate Sites", value: "altsite" },
			{ text: "Geo Regions", value: "georegion" },
			{ text: "Schools", value: "school" },
			{ text: "Students", value: "student" },
			{ text: "Trip Stops", value: "tripstop" },
			{ text: "Pool Stops", value: "poolStops" },
			{ text: "Map Address", value: "mapAddress" },
			{ text: "Address Point", value: "addressPoint" },
			{ text: "Unassigned Students", value: "unassignedStudentsRouting" }
		];
		...
		self.cardStyle = {
			...
			"poolStops": { title: "Pool Stops", color: "#040df7" },
			"mapAddress": { title: "Map Address", color: "#ff0000" },
			"addressPoint": { title: "Address Point", color: "#feff6f" },
			"unassignedStudentsRouting": { title: "Unassigned Students", color: "#3333ff" },
		};
	...
	RoutingMapSearch.prototype.getSuggestedResultByNormalType = function(type, value, count)
	{
		...
		if (type == "student")
		{
			queryString += "&studentLocation=true";
		}
		else if (type == "school")
		{
			queryString += "&schoolLocation=true";
		}

		return tf.promiseAjax.get(pathCombine(tf.api.apiPrefix(), "search", tf.dataTypeHelper.getEndpoint(type), "simple") + queryString, {}, { overlay: false })
			.then(function(data)
			{
					...
					var style = self.cardStyle[type],
						...
						}).map(function(item)
						{
							var entity = {
								...
								x: item.Xcoord,
								y: item.Ycoord,
								geometry: TF.xyToGeometry(item.Xcoord, item.Ycoord)
							};
							if (type == "student")
							{
								entity.subtitle = entity.subtitle + " , " + item.LocationName + "";
							}
							return entity;
						}),
