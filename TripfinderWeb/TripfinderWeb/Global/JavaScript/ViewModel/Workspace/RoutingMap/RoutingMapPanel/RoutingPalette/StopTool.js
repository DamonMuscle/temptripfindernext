(function()
{
	...
	*/
	...
	// StopTool.prototype.offsetInsetPoint = function(point, geometry)
	// {
	// 	var self = this;
	...
	// 	}
	// 	return self._findStreetByAddress(point.address).then(function(result)
	// 	{
	// 		var closestStreet = result.features[0].geometry;
	// 		var closestPointOnStreet = self._arcgis.geometryEngine.nearestCoordinate(closestStreet, geometry).coordinate;
	// 		var offsetLine = new self._arcgis.Polyline({ paths: [[geometry.x, geometry.y], [closestPointOnStreet.x, closestPointOnStreet.y]], spatialReference: { wkid: 102100 } });
	// 		var offsetLineDistance = self._arcgis.geometryEngine.geodesicLength(offsetLine, "meters");

	// 		var startPoint = new self._arcgis.Point({ x: closestStreet.paths[0][0][0], y: closestStreet.paths[0][0][1], spatialReference: {wkid:102100} });
	// 		var pointsCount = closestStreet.paths[0].length;
	// 		var endPoint = new self._arcgis.Point({ x: closestStreet.paths[0][pointsCount - 1][0], y: closestStreet.paths[0][pointsCount - 1][1], spatialReference: {wkid:102100} });
	// 		var toStartGeom = new self._arcgis.Polyline({ paths: [[startPoint.x, startPoint.y], [closestPointOnStreet.x, closestPointOnStreet.y]], spatialReference: { wkid: 102100 } });
	// 		var toEndGeom = new self._arcgis.Polyline({ paths: [[endPoint.x, endPoint.y], [closestPointOnStreet.x, closestPointOnStreet.y]], spatialReference: { wkid: 102100 } });
	// 		var toStartPDistance = self._arcgis.geometryEngine.geodesicLength(toStartGeom, "meters");
	// 		var toEndPDistance = self._arcgis.geometryEngine.geodesicLength(toEndGeom, "meters");
	...
	// 		{
	// 			return self.viewModel._viewModal.getJunctionStreetOnPoint(startPoint).then(function(startJunctions)
	// 			{
	// 				if (startJunctions && startJunctions.length > 1)
	// 				{

	// 					return getInOffPoint(closestStreet, startPoint, offsetLineDistance);
	// 				}
	// 			});
	// 		}
	// 		else if ((toEndPDistance < 15 && toStartPDistance >= 15))
	// 		{
	// 			return self.viewModel._viewModal.getJunctionStreetOnPoint(endPoint).then(function(endJunctions)
	// 			{
	// 				if (endJunctions && endJunctions.length > 1)
	// 				{
	// 					return getInOffPoint(closestStreet, endPoint, offsetLineDistance);
	// 					//return self._getPointOnPolylineByDistanceToPoint(offsetStreet, 15, false);
	// 				}
	// 			});
	// 		}
	// 		else if (toEndPDistance < 15 && toStartPDistance < 15)
	// 		{
	// 			return Promise.all([
	// 				self.viewModel._viewModal.getJunctionStreetOnPoint(startPoint),
	// 				self.viewModel._viewModal.getJunctionStreetOnPoint(endPoint)
	// 			]).then(function(data)
	// 			{
	...
	// 			});
	// 		}
	// 		function getInOffPoint(streetGeom, junctionPoint, offsetLineDistance)
	// 		{
	// 			var insetPoint = self._getInsetPointOnLine(junctionPoint, streetGeom, 15);

	// 			var _offsetDistance = offsetLineDistance > 10 ? offsetLineDistance : 10;
	// 			var buffer = self._arcgis.geometryEngine.geodesicBuffer(insetPoint, _offsetDistance, "meters");
	// 			var segment = new self._arcgis.Polyline({
	// 				type: "polyline",
	// 				spatialReference: {wkid:102100},
	// 				paths: [[[junctionPoint.x, junctionPoint.y], [insetPoint.x, insetPoint.y]]]
	// 			});
	// 			var anotherPoint = self._findPointOnLineByDistance(segment, 100);
	// 			segment = new self._arcgis.Polyline({
	// 				type: "polyline",
	// 				spatialReference: {wkid:102100},
	// 				paths: [[[anotherPoint.x, anotherPoint.y], [insetPoint.x, insetPoint.y]]]
	// 			});
	// 			var rotateSegment = self._isPointOnRightOfLine(segment, geometry) ? self._arcgis.geometryEngine.rotate(segment, 90, insetPoint) : self._arcgis.geometryEngine.rotate(segment, -90, insetPoint);
	...
	// StopTool.prototype._findStreetByAddress = function(address)
	// {
	// 	var self = this;
	...
	// 		where += " AND " + "City = '" + city + "'";
	// 	}
	// 	var queryTask = new self._arcgis.QueryTask(self.streetUrl);
	// 	var query = new self._arcgis.Query();
	// 	query.where = where;
